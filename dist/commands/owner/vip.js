    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (subcommand === 'add') {
            ...
            if (botConfig.vipUsers.includes(targetUser.id)) {
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.error).setDescription(`${config.emojis.error} **${targetUser.tag}** is already a **VIP**.`)] });
            }
        ...
        if (subcommand === 'remove') {
            const targetUser = interaction.options.getUser('user', true);
            if (!botConfig.vipUsers.includes(targetUser.id)) {
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.error).setDescription(`${config.emojis.error} **${targetUser.tag}** is not a **VIP**.`)] });
            }
        ...
        if (subcommand === 'list') {
            ...
            if (users.length === 0)
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.warning).setDescription(`${config.emojis.warning} No VIPs found.`)] });
        ...
        const embed = embedStyle('VIP Commands', `\`${config.prefix}vip add <user>\`\n` +
