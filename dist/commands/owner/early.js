    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (subcommand === 'add') {
            ...
            if (botConfig.earlyUsers.includes(targetUser.id)) {
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.error).setDescription(`${config.emojis.error} **${targetUser.tag}** is already an **Early Supporter**.`)] });
            }
        ...
        if (subcommand === 'remove') {
            const targetUser = interaction.options.getUser('user', true);
            if (!botConfig.earlyUsers.includes(targetUser.id)) {
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.error).setDescription(`${config.emojis.error} **${targetUser.tag}** is not an **Early Supporter**.`)] });
            }
        ...
        if (subcommand === 'list') {
            ...
            if (users.length === 0)
                return interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(config.colors.warning).setDescription(`${config.emojis.warning} No Early Supporters found.`)] });
        ...
        const embed = embedStyle('Early User Commands', `\`${config.prefix}early add <user>\`\n` +
