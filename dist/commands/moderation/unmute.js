...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (user.id === interaction.user.id) {
            yield interaction.reply({ content: `${config.emojis.error} **You cannot unmute yourself.**`, ephemeral: true });
            return;
        }
        if (!(0, permission_1.canModerate)(interaction.member, user, discord_js_1.PermissionFlagsBits.ModerateMembers)) {
            yield interaction.reply({ content: `${config.emojis.error} **You cannot moderate this user due to role hierarchy.**`, ephemeral: true });
            return;
        }
        if (!user.isCommunicationDisabled()) {
            yield interaction.reply({ content: `${config.emojis.error} **This user is not muted.**`, ephemeral: true });
            return;
        }
        ...
        try {
            ...
            console.error(error);
            yield interaction.editReply({ content: `${config.emojis.error} **Failed to unmute user.**` });
        }
    });
}
