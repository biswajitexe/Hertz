...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!interaction.guild)
            return;
        yield interaction.reply({ content: `${config.emojis.loading || "🔄"} Fetching bans...`, ephemeral: false });
        try {
            ...
            for (const ban of bans.values()) {
                yield interaction.guild.members.unban(ban.user.id, `Unban All by ${interaction.user.tag}`)
                    .then(() => count++)
                    .catch(() => errors++);
            }
