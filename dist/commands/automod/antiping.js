...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!isOwner && !isExtraOwner && !isExtraAdmin && !isBotOwner) {
            yield interaction.reply({ content: `${config.emojis.error} **Only the Server Owner, Trustable Admins, or Bot Owner can manage anti-ping settings.**`, ephemeral: true });
            return;
        }
        ...
        try {
            if (sub === 'enable') {
                if (guildData.messageFilters.massMention) {
                    yield interaction.editReply({ content: `${config.emojis.error} **Anti-Ping is already enabled!**` });
                    return;
                }
                guildData.messageFilters.massMention = true;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply({ content: `${config.emojis.success} **Anti-Ping (Mass Mention) filter has been Enabled.**` });
            }
            else if (sub === 'disable') {
                if (!guildData.messageFilters.massMention) {
                    yield interaction.editReply({ content: `${config.emojis.error} **Anti-Ping is already disabled!**` });
                    return;
                }
                guildData.messageFilters.massMention = false;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply({ content: `${config.emojis.success} **Anti-Ping (Mass Mention) filter has been DISABLED.**` });
            }
            else if (sub === 'status') {
