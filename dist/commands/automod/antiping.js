...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!isOwner && !isExtraOwner && !isExtraAdmin && !isBotOwner) {
            yield interaction.reply((0, componentV2_1.createErrorV2)('Only the Server Owner, Trustable Admins, or Bot Owner can manage anti-ping settings.').toPayload({ ephemeral: true }));
            return;
        }
        ...
        try {
            if (sub === 'enable') {
                if (guildData.messageFilters.massMention) {
                    yield interaction.editReply((0, componentV2_1.createErrorV2)('Anti-Ping is already enabled!').toPayload());
                    return;
                }
                guildData.messageFilters.massMention = true;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply((0, componentV2_1.createSuccessV2)('Anti-Ping (Mass Mention) filter has been Enabled.').toPayload());
            }
            else if (sub === 'disable') {
                if (!guildData.messageFilters.massMention) {
                    yield interaction.editReply((0, componentV2_1.createErrorV2)('Anti-Ping is already disabled!').toPayload());
                    return;
                }
                guildData.messageFilters.massMention = false;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply((0, componentV2_1.createSuccessV2)('Anti-Ping (Mass Mention) filter has been DISABLED.').toPayload());
            }
            else if (sub === 'status') {
                ...
                const embed = new componentV2_1.V2Embed()
                    ...
                    .setDescription(`> \`${config.prefix}antiping enable\`\n` +
