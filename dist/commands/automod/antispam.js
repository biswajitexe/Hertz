...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!isOwner && !isExtraOwner && !isExtraAdmin && !isBotOwner) {
            yield interaction.reply({ content: `${config.emojis.error} **Only the Server Owner, Trustable Admins, or Bot Owner can manage anti-spam settings.**`, ephemeral: true });
            return;
        }
        ...
        try {
            if (sub === 'enable') {
                if (guildData.messageFilters.spam) {
                    yield interaction.editReply({ content: `${config.emojis.error} **Anti-Spam is already enabled!**` });
                    return;
                }
                guildData.messageFilters.spam = true;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply({ content: `${config.emojis.success} **Anti-Spam filter has been Enabled.**` });
            }
            else if (sub === 'disable') {
                if (!guildData.messageFilters.spam) {
                    yield interaction.editReply({ content: `${config.emojis.error} **Anti-Spam is already disabled!**` });
                    return;
                }
                guildData.messageFilters.spam = false;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply({ content: `${config.emojis.success} **Anti-Spam filter has been DISABLED.**` });
            }
            else if (sub === 'status') {
