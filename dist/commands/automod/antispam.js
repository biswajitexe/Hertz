...
function run(interaction, database) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!isOwner && !isExtraOwner && !isExtraAdmin && !isBotOwner) {
            yield interaction.reply((0, componentV2_1.createErrorV2)('Only the Server Owner, Trustable Admins, or Bot Owner can manage anti-spam settings.').toPayload({ ephemeral: true }));
            return;
        }
        ...
        try {
            if (sub === 'enable') {
                if (guildData.messageFilters.spam) {
                    yield interaction.editReply((0, componentV2_1.createErrorV2)('Anti-Spam is already enabled!').toPayload());
                    return;
                }
                guildData.messageFilters.spam = true;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply((0, componentV2_1.createSuccessV2)('Anti-Spam filter has been Enabled.').toPayload());
            }
            else if (sub === 'disable') {
                if (!guildData.messageFilters.spam) {
                    yield interaction.editReply((0, componentV2_1.createErrorV2)('Anti-Spam is already disabled!').toPayload());
                    return;
                }
                guildData.messageFilters.spam = false;
                yield database.insertGuild(interaction.guild.id, guildData);
                yield interaction.editReply((0, componentV2_1.createSuccessV2)('Anti-Spam filter has been DISABLED.').toPayload());
            }
            else if (sub === 'status') {
                ...
                const embed = new componentV2_1.V2Embed()
                    ...
                    .setDescription(`> \`${config.prefix}antispam enable\`\n` +
