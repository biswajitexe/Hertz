...
export async function run(interaction: ChatInputCommandInteraction, database: Database) {
    ...
    if (channel.isThread()) {
        return interaction.reply(createErrorV2("Threads cannot be locked individually via this command.").toPayload({ ephemeral: true }));
    }
    ...
    if (currentOverwrites && currentOverwrites.deny.has(PermissionFlagsBits.SendMessages)) {
        return interaction.reply(createErrorV2("**This channel is already locked.**").toPayload({ ephemeral: true }));
    }
