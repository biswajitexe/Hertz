...
export async function run(interaction: ChatInputCommandInteraction, database: Database) {
    ...
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.ModerateMembers) && interaction.user.id !== process.env.OWNER_ID) {
        await interaction.reply({ embeds: [createErrorEmbed(interaction.user, "**You do not have permission to timeout members.**")], ephemeral: true });
        return;
    }
    ...
    if (user.id === interaction.user.id) {
        await interaction.reply({ embeds: [createErrorEmbed(interaction.user, "**You cannot timeout yourself.**")], ephemeral: true });
        return;
    }
    if (user.id === interaction.client.user.id) {
        await interaction.reply({ embeds: [createErrorEmbed(interaction.user, "**You cannot timeout me.**")], ephemeral: true });
        return;
    }
    if (user.id === interaction.guild.ownerId) {
        await interaction.reply({ embeds: [createErrorEmbed(interaction.user, "**You cannot timeout the server owner.**")], ephemeral: true });
        return;
    }
    if (!user.moderatable) {
        await interaction.reply({ embeds: [createErrorEmbed(interaction.user, "**I cannot timeout this user. My role is likely below theirs.**")], ephemeral: true });
        return;
    }
    ...
    try {
        ...
        console.error(error);
        await interaction.editReply({ embeds: [createErrorEmbed(interaction.user, "**Failed to timeout user.**")] });
    }
}