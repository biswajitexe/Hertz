...
export const command = new SlashCommandBuilder()
    ...
    .addStringOption(option => option
        .setName('duration')
        .setDescription('Duration (e.g. 1h, 30m, 1d) [Default: 5m].')
        .setRequired(false)
    )
    .addStringOption(option => option
        .setName('reason')
...
export async function run(interaction: ChatInputCommandInteraction, database: Database) {
    ...
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.ModerateMembers) && interaction.user.id !== process.env.OWNER_ID) {
        await interaction.reply(createErrorEmbed(interaction.user, "**You do not have permission to timeout members.**").toPayload({ ephemeral: true }));
        return;
    }
    ...
    if (user.id === interaction.user.id) {
        await interaction.reply(createErrorEmbed(interaction.user, "**You cannot timeout yourself.**").toPayload({ ephemeral: true }));
        return;
    }
    if (user.id === interaction.client.user.id) {
        await interaction.reply(createErrorEmbed(interaction.user, "**You cannot timeout me.**").toPayload({ ephemeral: true }));
        return;
    }
    if (user.id === interaction.guild.ownerId) {
        await interaction.reply(createErrorEmbed(interaction.user, "**You cannot timeout the server owner.**").toPayload({ ephemeral: true }));
        return;
    }
    if (!user.moderatable) {
        await interaction.reply(createErrorEmbed(interaction.user, "**I cannot timeout this user. My role is likely below theirs.**").toPayload({ ephemeral: true }));
        return;
    }
    ...
    try {
        ...
        await logAction(interaction.guild, user.user, interaction.user, 'TIMEOUT', reason, database, extraInfo);

        // Success Reply
        const successEmbed = createSuccessEmbed(interaction.user, `**Timed Out ${user.user.tag}**`)
        ...
        console.error(error);
        await interaction.editReply(createErrorEmbed(interaction.user, "**Failed to timeout user.**").toPayload());
    }
}